...
final class CircularDrawingDelegate extends DrawingDelegate<CircularProgressIndicatorSpec> {
    ...
    public void fillIndicator(Canvas canvas, Paint paint, float startFraction, float endFraction, int color) {
        ...
        if (startFraction != endFraction) {
            ...
            if (endFraction >= startFraction) {
                f = (endFraction - startFraction) * 360.0f * ((float) this.arcDirectionFactor);
            } else {
                f = ((endFraction + 1.0f) - startFraction) * 360.0f * ((float) this.arcDirectionFactor);
            }
            float arcDegree = f;
            canvas.drawArc(new RectF(-this.adjustedRadius, -this.adjustedRadius, this.adjustedRadius, this.adjustedRadius), startDegree, arcDegree, false, paint);
    ...
    public void fillTrack(Canvas canvas, Paint paint) {
        ...
        canvas.drawArc(new RectF(-this.adjustedRadius, -this.adjustedRadius, this.adjustedRadius, this.adjustedRadius), 0.0f, 360.0f, false, paint);
    }
    ...
    private void drawRoundedEnd(Canvas canvas, Paint paint, float trackSize, float cornerRadius, float positionInDeg) {
        ...
        canvas.drawRoundRect(new RectF(this.adjustedRadius - (trackSize / 2.0f), cornerRadius, this.adjustedRadius + (trackSize / 2.0f), -cornerRadius), cornerRadius, cornerRadius, paint);
        canvas.restore();
    }
}
