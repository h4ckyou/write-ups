...
public final class Cam16 {
    ...
    static Cam16 fromXyzInViewingConditions(double x, double y, double z, ViewingConditions viewingConditions) {
        ...
        double atanDegrees = Math.toDegrees(Math.atan2(bAF2, a));
        if (atanDegrees < 0.0d) {
        ...
        double d9 = bAF2;
        double d10 = rA;
        double alpha = Math.pow(1.64d - Math.pow(0.29d, viewingConditions.getN()), 0.73d) * Math.pow((Math.hypot(a, bAF2) * (((3846.153846153846d * ((Math.cos(Math.toRadians(hue2 < 20.14d ? hue2 + 360.0d : hue2) + 2.0d) + 3.8d) * 0.25d)) * viewingConditions.getNc()) * viewingConditions.getNcb())) / (u + 0.305d), 0.9d);
