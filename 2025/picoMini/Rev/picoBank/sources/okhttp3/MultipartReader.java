        ...
        public long read(Buffer sink, long byteCount) {
            ...
            if (!(j3 >= 0)) {
                ...
                if (this_$iv.hasDeadline()) {
                    ...
                    try {
                        long limit = MultipartReader.this.currentPartBytesRemaining(j3);
                        if (limit == 0) {
                            j2 = -1;
                        } else {
                            j2 = MultipartReader.this.source.read(buffer, limit);
                        }
                        return j2;
                    } finally {
                    ...
                    try {
                        long limit2 = MultipartReader.this.currentPartBytesRemaining(j3);
                        if (limit2 == 0) {
                            j = -1;
                        } else {
                            j = MultipartReader.this.source.read(buffer, limit2);
                        }
                        return j;
                    } finally {
                        this_$iv.timeout(originalTimeout$iv, TimeUnit.NANOSECONDS);
