...
final class EnumEntriesList<T extends Enum<T>> extends AbstractList<T> implements EnumEntries<T>, Serializable {
    ...
    public EnumEntriesList(Function0<T[]> entriesProvider2) {
        Intrinsics.checkNotNullParameter(entriesProvider2, "entriesProvider");
        this.entriesProvider = entriesProvider2;
    }

    private final T[] getEntries() {
        Enum[] e = this._entries;
        if (e != null) {
            return e;
        }
        Enum[] e2 = this.entriesProvider.invoke();
        this._entries = e2;
        return e2;
    }
    ...
    public boolean contains(T element) {
        ...
        return ((Enum) ArraysKt.getOrNull((T[]) getEntries(), element.ordinal())) == element;
    }

    public int indexOf(T element) {
        Intrinsics.checkNotNullParameter(element, "element");
        int ordinal = element.ordinal();
        if (((Enum) ArraysKt.getOrNull((T[]) getEntries(), ordinal)) == element) {
            return ordinal;
        }
        return -1;
    }
